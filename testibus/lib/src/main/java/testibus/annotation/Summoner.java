...
class SummonerImpl<A extends Annotation, P, S> implements Summoner<P, S>, Summoner.StewardSummoner<P, S> {
    ...
    Optional<A> lookupAnnotation() {
        ...
        Optional<A> result = element.flatMap(e -> AnnotationSupport.findAnnotation(e, annotationType));
    ...
    Optional<List<A>> lookupAnnotations() {
        final Optional<AnnotatedElement> element = context.getElement();
        Optional<List<A>> result = element.map(e -> AnnotationSupport.findRepeatableAnnotations(e, annotationType)).filter(l -> l.size() > 0);
        if (result.isPresent() || element.filter(testClass::equals).isPresent()) return result;
        return Optional.of(AnnotationSupport.findRepeatableAnnotations(testClass, annotationType)).filter(l -> l.size() > 0);
    }
}
